module Content
    ...
    function topic_replies(
        ...
        params = _opts_to_dict(opts)
        resp = ApiResponse(Client.http_get(ctx.config, "/v1/content/topics/$(String(topic_id))/comments"; params))
        _check(resp)
        items = if haskey(resp.data, :items) && !isnothing(resp.data.items)
    ...
    function create_topic_reply(
        ...
        body = _opts_to_dict(opts)
        resp = ApiResponse(Client.http_post(ctx.config, "/v1/content/topics/$(String(topic_id))/comments"; body))
        _check(resp)
